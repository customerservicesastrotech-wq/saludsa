...
function sleepMsgs() {
  ...
  out.push(`La tablet vuelve a estar disponible a las ${SLC().end}. Descansar es parte del plan.`);
...
async function sleepNow() {
  ...
  try { await syncShield(); const r = await SH.sleepNow({ until: sleepNextEnd() }); toast(`Buenas noches. Bloqueada hasta las ${hhmm(new Date((r && r.until) || sleepNextEnd()))}`); }
...
async function sleepWarnSchedule() {
  ...
  try {
    ...
    await LN.schedule({ notifications: days.map(g => ({ id: 311 + g, title: 'Plan 20', body: `En 10 min empieza el modo dormir (hasta las ${c.end})`, schedule: { on: { weekday: ((t > h * 60 + m ? g + 6 : g) % 7) + 1, hour: Math.floor(t / 60), minute: t % 60 }, allowWhileIdle: true } })) });
  } catch (e) { console.warn(e); }
}
/* En la agenda: la hora de dormir de cada noche */
function sleepVirtual(k) { return sleepOnDay(k) ? { vid: 'sleep', date: k, time: SLC().start, title: `Modo dormir · la tablet se bloquea hasta las ${SLC().end}`, kind: 'sueno', note: '' } : null; }
...
function sleepStatusText() {
  ...
  return sleepOnDay(today()) ? `Esta noche: de ${c.start} a ${c.end} del día siguiente.` : `Esta noche no toca (día sin modo dormir). Próxima: ${cap1(fmt((() => { let d = addDays(today(), 1); for (let i = 0; i < 7 && !sleepOnDay(d); i++) d = addDays(d, 1); return d; })()))} a las ${c.start}.`;
}
function sleepCard() {
  const c = SLC(), nat = !!SHN();
  ...
  return `<div class="card c12 ${c.on ? 'accent' : ''}" id="sleepcard"><div class="row between"><h3 style="margin:0">🌙 Modo dormir</h3><div class="seg"><button data-a="slon" data-x="1" class="${c.on ? 'on' : ''}">Activado</button><button data-a="slon" data-x="0" class="${!c.on ? 'on' : ''}">Desactivado</button></div></div>
    <p class="small" style="margin-top:8px">A tu hora de dormir, la tablet queda <b>bloqueada hasta la mañana</b>: una pantalla oscura cubre cualquier app. Las <b>llamadas y la emergencia siempre funcionan</b>.</p>
    <p style="margin:6px 0 12px"><b>${esc(sleepStatusText())}</b></p>
    <div class="slgrid">
      <div class="field"><div class="lbl">Horario</div><div class="row"><input type="time" data-sl="start" value="${c.start}" style="width:130px"> a <input type="time" data-sl="end" value="${c.end}" style="width:130px"></div></div>
    ...
    <div class="row" style="margin-top:6px"><button class="btn pri" data-a="slnow" ${nat ? '' : 'disabled'}>🌙 Dormir ahora (hasta las ${c.end})</button></div>
...
bjCommand = function (text) {
  ...
  if (direct) {
    ...
    return { text: `Buenas noches. Bloqueo la tablet hasta las **${hhmm(new Date(until))}**. Si surge algo urgente, el botón rojo llama. Descansa.`, after: () => setTimeout(() => sleepNow(), 2500) };
  }
  return _slCmd(text);
};
...
window.sleep = { native: sleepNative, nextEnd: sleepNextEnd, onDay: sleepOnDay, msgs: sleepMsgs, warn: sleepWarnSchedule, virtual: sleepVirtual };
